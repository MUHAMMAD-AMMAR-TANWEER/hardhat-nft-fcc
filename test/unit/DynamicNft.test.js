...
!developmentChains.includes(network.name)
    ...
    : describe("Dynamic Nft", function () {
          let deployer, MockV3Aggregator, DynamicNft
          beforeEach(async () => {
          ...
          describe("constructor", function () {
              it("Initialize all variables correctly", async function () {
          ...
          describe("Mint NFT", function () {
              it("emits event on deployment", async function () {
                  // const mintNft = await DynamicNft.mintNft("2000")
                  const fee = ethers.utils.parseEther("1")
                  await expect(DynamicNft.mintNft(fee)).to.emit(DynamicNft, "CreatedNFT")
                  const tokenCounter = await DynamicNft.getTokenCounter()
                  assert.equal(tokenCounter.toString(), "1")
                  const tokenUri = await DynamicNft.tokenURI(0)
                  assert.equal(tokenUri, highTokenUri)
              })
              it("Sets the lower Image when the price is too high", async function () {
